...
However further investigation shows that automatic transmission gives better fuel consumption cars that weigh less/have less horspower and manual transmission is better for cars that weigh more/have more power. Summarised in the following table.
...
First we create a fit to test all of the adjusted models.
```{r}
fit <- lm(mpg ~ ., mtcars)
```

Scatterplots can be generated using the _pairs_ routine (output not included to save space). It appears the predictors most correlated wth _mpg_ (miles per gallon) are _wt_ (weight), _cyl_ (no. cylinders), _disp_ (displacement), and _hp_ (horsepower). In the [adjusted fits] in the appendix from this model `lm(mpg ~ ., mtcars)`, only _wt_ was even slightly correlated after effects of all other fits were taken out of predictor and outcome. We will try the models in the table below. A sample summary from one of the models can be found in the appendix. Summary from all models is not included. The reader is invited to create these summaries.
...
```
Output from [anova] in the appendix shows us also that the extras terms do not help. We could also use vif to test this (but space constraints mean we won't!).
...
[fit summaries]: #fitsummaries
coefficients for model mpg ~ wt + hp + disp
```{r, echo = FALSE}
...
[transmission summary]: #transmissionsummary
```{r}
